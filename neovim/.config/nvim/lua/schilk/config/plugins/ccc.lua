...
function M.config()
	-- Multi-color-space picker. Taken from:
	-- https://github.com/uga-rosa/ccc.nvim/wiki/Use-multiple-color-spaces-simultaneously
	...
	local RgbHslCmykInput = setmetatable({
		...
		bar_name = { "R", "G", "B", "Hue", "Sat", "Light", "C", "M", "Y", "K" },
	}, { __index = ColorInput })
    ...
    vim.keymap.set("n", "<leader>mc", ":CccPick<CR>", { silent = true, desc = "🎨 Pick Color." })
    vim.keymap.set("n", "<leader>mC", ":CccHighlighterToggle<CR>", { silent = true, desc = "🎨 Toggle Color highlight." })
end
